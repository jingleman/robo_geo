...
extra_compile_args += ["-Wall", "-Wextra", "-std=c++11"]
	# Didn't seem to speed up the Visibility_Polygon contruction.
extra_compile_args += ["-g3", "-O0", "-DDEBUG=9", "-UNDEBUG"]
	# Commenting this out didn't seem to speed up the Visibility_Polygon contruction.

visilibity_src = os.environ['VISILIBITY_SRC']
...
print([numpy.get_include(), '${VISILIBITY_SRC}/'])
